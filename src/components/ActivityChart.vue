...
export default {
  ...
  methods: {
      ...
      return {
            ...
            callbacks: {
              ...
              label: (context) => {
                if (context.datasetIndex === 0) {
                  ...
                  return `Calories: ${Math.round(context.parsed.y)} cal`
                }
              }
            }
          }
        ...
        scales: {
          y: {
            ...
            position: 'left',
            beginAtZero: true,
            max: 3, // 3 heures max pour la durée
            title: {
              display: true,
    ...
    prepareChartData() {
      ...
      // Préparer les données de durée (en heures)
      const durationData = this.chartData.map(item => {
        return item.total_duration > 0 ? (item.total_duration / 60).toFixed(1) : 0
      })

      // Préparer les données de calories
      const caloriesData = this.chartData.map(item => {
        return item.total_calories > 0 ? item.total_calories : 0
      })
