...
export default {
  ...
  methods: {
            ...
            callbacks: {
              ...
              label: (context) => {
                if (context.datasetIndex === 0) {
                  return `Humeur moyenne: ${context.parsed.y.toFixed(1)}/10`
                } else {
                  return `Nombre d'entrées: ${context.parsed.y}`
                }
              }
            }
          }
    ...
    prepareChartData() {
      ...
      // Préparer les données de score moyen
      const scoreData = this.chartData.map(item => {
        return item.avg_score > 0 ? item.avg_score.toFixed(1) : null
      })

      // Préparer les données de nombre d'entrées
      const countData = this.chartData.map(item => {
        return item.count || 0
      })
